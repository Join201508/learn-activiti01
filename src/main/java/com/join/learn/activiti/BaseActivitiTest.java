...
public class BaseActivitiTest {
    ...
    public ProcessInstance startProcessByKey(String processDefinitionKey, Map<String, Object> vars) {
        ProcessInstance pi = engine.getRuntimeService().startProcessInstanceByKey(processDefinitionKey, vars);// processDefinitionKey即流程配置bpmn文件的process节点的ID
        printProcessInstance(pi);
        return pi;
    }
    ...
    public void startProcessById(String processDefinitionId) {
        ProcessInstance pi = engine.getRuntimeService().startProcessInstanceById(processDefinitionId);
        printProcessInstance(pi);
    }
    ...
    public void printTaskListByExecId(String executionId) {
        List<Task> taskList = engine.getTaskService().createTaskQuery().executionId(executionId).list();
        if (taskList.size() == 0) {
    ...
    private void printTask(List<Task> taskList) {
        for (Task t : taskList) {
            System.out.println("  taskid:" + t.getId() + "  executionId:" + t.getExecutionId() + "  name:" + t.getName()
                + "  proDefId:" + t.getProcessDefinitionId() + "  assignee:" + t.getAssignee());
        }
    }
    ...
    private void printProcessDefinition(RepositoryService service, Deployment deployment) {
        ...
        ProcessDefinition definition = service.createProcessDefinitionQuery().deploymentId(deployment.getId()).singleResult();
