...
public class CandidateGroupsTest extends BaseActivitiTest {
    ...
    public void queryGroupUsers() {
        ...
        for (IdentityLink idLink : list) {
            System.out.println("groupId:" + idLink.getGroupId() + " , userId:" + idLink.getUserId() + " , taskId:"
                + idLink.getTaskId() + " , type:" + idLink.getType());
        }
    }

    /**
     * userTask设置了【候选组】的，通过【候选组】查询任务： activiti自动关联如下三张表查询出来： act_ru_identitylink 任务与候选人(候选组)关系表
    ...
    public void queryTaskByCandidateGroup() {
        ...
        List<Task> list = engine.getTaskService().createTaskQuery().taskCandidateGroup(groupId).list();
        if (list.size() == 0) {
    ...
    /**
     * userTask设置了【候选组】的，通过【候选人】也可以查询到任务： activiti自动关联如下三张表查询出来： act_ru_identitylink 任务与候选人(候选组)关系表
     * act_id_user、 act_id_group、 act_id_membership
     */
    @Test
    public void queryTaskByCandidateUser() {
        ...
        List<Task> list = engine.getTaskService().createTaskQuery().taskCandidateUser(userId).list();
        if (list.size() == 0) {
    ...
    /**
     * userTask设置了【候选组】的，通过assignee查询不到数据， 除非指定了assignee人
     */
    @Test
    public void queryTaskByAssignee() {
        String userId = "刘德华经理";
        List<Task> list = engine.getTaskService().createTaskQuery().taskAssignee(userId).list();
        if (list.size() == 0) {
    ...
    private void printTask(List<Task> list) {
        if (list.size() > 0) {
            ...
            for (Task t : list) {
                System.out.println("taskId=" + t.getId() + " , name=" + t.getName() + " , DefinitionKey=" + t.getTaskDefinitionKey()
