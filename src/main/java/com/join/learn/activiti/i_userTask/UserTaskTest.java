...
public class UserTaskTest extends BaseActivitiTest {
    ...
    @Test
    public void deploy() {
        super.deploy("UserTask节点示例流程3", "UserTaskProcess.bpmn", "UserTaskProcess.png");
    }
    ...
    public void start() {
        ...
        ProcessInstance pi = engine.getRuntimeService().startProcessInstanceByKey(processKey, vars);
        super.printProcessInstance(pi);

        // 查询task1
        Task task1 = engine.getTaskService().createTaskQuery().processInstanceId(pi.getProcessInstanceId()).singleResult();
    ...
    public void completeTask1() {
        ...
        // 查询下一步任务:usertask2
        Task task2 = engine.getTaskService().createTaskQuery().executionId(executionId).singleResult();
